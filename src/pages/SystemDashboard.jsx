...
const API_URIS = {
  local: 'http://localhost:8 000',
  deployed: 'https://dmcashield- agency-8 666. apps. hostingguru. io'
};

const getApiUrl = () => {
  if (import. meta. env. VITE_ API_ URL) return import. meta. env. VITE_ API_ URL;
  if (import. meta. env. PROD === 'true') return API_ URIS. deployed;
  return API_ URIS. local;
};
...
export default function SystemDashboard() {
  ...
  const demoData = {
    ...
    departments: {
      ...
      email_ sending: 'online', tracking: 'online', sales: 'online'
    },
    stats: { total_ leads: 247, hot_ leads: 38, open_ rate: 28, reply_ rate: 9, tasks_ active: 7, campaigns: 4 }
  };
    ...
    <div className="min-h- screen bg-gray-900 text-white p-4 md:p-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-gray-800 p-4 md:p-6 rounded-xl border border-blue-500/50">
          <div className="text-2xl md:text-3xl font-bold text-blue-400">{stats. total_ leads || 0}</div>
        ...
        <div className="bg-gray-800 p-4 md:p-6 rounded-xl border border-red-500/50">
          <div className="text-2xl md:text-3xl font-bold text-red-400">{stats. hot_ leads || 0}</div>
        ...
        <div className="bg-gray-800 p-6 rounded-xl">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center p-2 bg-gray-700/50 rounded">
              ...
              <span className="text-green-400 font-bold text-xl">{stats. open_ rate || 0}%</span>
            </div>
            <div className="flex justify-between items-center p-2 bg-gray-700/50 rounded">
              <span className="text-gray-400">Reply Rate</span>
              <span className="text-green-400 font-bold text-xl">{stats. reply_ rate || 0}%</span>
            </div>
            <div className="flex justify-between items-center p-2 bg-gray-700/50 rounded">
              <span className="text-gray-400">Active Tasks</span>
              <span className="text-blue-400 font-bold text-xl">{stats. tasks_ active || 0}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-800 p-6 rounded-xl">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center p-2 bg-gray-700/50 rounded">
              ...
              <span className="text-blue-400 font-bold text-xl">{leads. length || stats. total_ leads || 0}</span>
            </div>
            <div className="flex justify-between items-center p-2 bg-gray-700/50 rounded">
              <span className="text-gray-400">Hot Leads</span>
              <span className="text-red-400 font-bold text-xl">{stats. hot_ leads || 0}</span>
            </div>
            <div className="flex justify-between items-center p-2 bg-gray-700/50 rounded">
              <span className="text-gray-400">Scored (70+)</span>
              <span className="text-green-400 font-bold text-xl">{leads. filter(l => l. lead_ score >= 70)?. length || 0}</span>
            </div>
          </div>
        </div>
      </div>
